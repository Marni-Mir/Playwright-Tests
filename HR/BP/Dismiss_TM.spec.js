...
test.describe('Dismiss TM test', () => {
    ...
    test('Dismiss TM test flow', async ({ page, links }) => {
        ...
        // Заполняем поля 
        await bpFrame.locator(SELECTORS_CATALOG.TeamMemberCard.BP.dismissalDate).fill('03/25/2025 06:41:00 pm');
        await bpFrame.locator(SELECTORS_CATALOG.TeamMemberCard.BP.accessDeactivationDate).fill('03/27/2025 11:41:00 pm');
        await bpFrame.locator(SELECTORS_CATALOG.TeamMemberCard.BP.dismissalReason).selectOption('Fired');
        await bpFrame.locator(SELECTORS_CATALOG.TeamMemberCard.BP.transferFiles).selectOption('No');
 ...
 */  
        ...
        // 6. Скриншот успеха
        await ScreenshotSuccess(page, 'Dismiss_TM', 'Dismiss_TM_BP');
    });
});
